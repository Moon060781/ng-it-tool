...
$status_msg = "";
$conn = new mysqli(DB_SERVER, AI_DB_USER, AI_DB_PASS, AI_DB_NAME);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_save_key']) && !$conn->connect_error) {
    ...
    if (!empty($key_name) && !empty($api_key)) {
        ...
        if ($stmt->execute()) {
            ...
            $status_msg = "<div class='alert error'>خرابی: " . $stmt->error . "</div>";
        }
        $stmt->close();
    }
}

$saved_profiles = (!$conn->connect_error) ? mysqli_query($conn, "SELECT id, key_name, platform_name FROM {$table_name} WHERE is_active = 1 ORDER BY id DESC") : null;
...
<div class="main-dashboard-wrapper">
    ...
    <div class="workspace-container">
        ...
        <details <?php echo (empty($status_msg)) ? '' : 'open'; ?>>
            ...
            <form method="POST" action="" style="margin-top: 12px;">
                <div class="form-group-grid">
                    ...
                    <div><label>پلیٹ فارم:</label><select name="platform_name"><option value="Google Gemini">Google Gemini</option><option value="OpenAI">OpenAI</option><option value="Groq Cloud">Groq Cloud</option></select></div>
                </div>
                <div class="form-group-grid">
        ...
        <div class="form-group-grid">
            <div><label>کلید منتخب کریں:</label><select id="selectedProfileId"><?php if ($saved_profiles && mysqli_num_rows($saved_profiles) > 0) { while ($row = mysqli_fetch_assoc($saved_profiles)) { echo "<option value='".intval($row['id'])."'>".htmlspecialchars($row['key_name'])." [".htmlspecialchars($row['platform_name'])."]</option>"; } } else { echo "<option value=''>کوئی کلید نہیں ملی۔</option>"; } ?></select></div>
