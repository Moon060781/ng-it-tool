...
<style>
  ...
  *{box-sizing:border-box}
  body{background:linear-gradient(135deg,#0f172a 0%,#1e293b 60%,#0f172a 100%);min-height:100vh;color:#e2e8f0;font-family:'Segoe UI',system-ui,sans-serif;font-size:0.95rem}
  ...
  .btn{display:inline-flex;align-items:center;gap:.35rem;padding:.5rem 1rem;border-radius:.6rem;font-weight:600;font-size:0.9rem;cursor:pointer;transition:all .2s;border:none}
  ...
  .btn-sm{padding:.35rem .7rem;font-size:0.85rem}
  textarea,input[type=text]{background:rgba(30,41,59,.8);border:1px solid rgba(100,116,139,.4);color:#e2e8f0;border-radius:.6rem;padding:.5rem 0.8rem;width:100%;font-size:0.95rem;transition:border .2s;resize:vertical}
...
<nav class="glass-dark border-b border-slate-700/50 sticky top-0 z-50">
  <div class="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-2 flex-wrap">
    <div class="flex items-center gap-2">
      ...
      <div>
        <div class="font-bold text-white text-sm leading-none">Deploy Manager</div>
        <div class="text-slate-400 text-xs">it.noorgee.com</div>
      </div>
    </div>
    <div class="flex items-center gap-2 flex-wrap">
      <a href="<?=SITE_URL?>" target="_blank" class="btn btn-slate btn-sm"><i class="fas fa-globe"></i> Live</a>
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <form method="POST" class="inline">
        ...
        <button type="submit" class="btn btn-orange btn-sm"
          onclick="return confirm('⚠️ Force Pull will RESET local changes. Continue?')">
      ...
      <form method="POST" class="inline">
        ...
        <button type="submit" class="btn btn-red btn-sm"
          onclick="return confirm('⚠️ Undo will discard ALL uncommitted changes. Continue?')">
      ...
      <form method="POST" class="inline">
        ...
        <button type="submit" class="btn btn-red btn-sm"
          onclick="return confirm('⚠️ This will REVERT the last commit. Continue?')">
...
<div class="glass rounded-lg p-3 flex flex-wrap gap-3 items-center justify-between text-xs">
  ...
  <div class="text-slate-500 text-xs">
    <i class="fas fa-robot mr-1"></i> Claude AI · <?=karachi_fmt()?>
  </div>
</div>
